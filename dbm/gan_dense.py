...
from dbm.mol_generator2 import Mol_Generator
...
class DS(Dataset):
    def __init__(self, data, cfg, train=True):
        ...
        g_out = Mol_Generator(data, train=train, res='out', rand_rot=False)
        g_inp = Mol_Generator(data, train=train, res='inp', rand_rot=False)

        self.elems_out = g_out.all_elems()
        self.elems_inp = g_inp.all_elems()
    ...
    def __len__(self):
        return min(len(self.elems_out), len(self.elems_inp))
    ...
    def __getitem__(self, ndx):
        ...
        pos_intra_inp = np.reshape(np.dot(d_inp['positions_intra'], R.T), (-1,))
        pos_inter_inp = np.reshape(np.dot(d_inp['positions_inter'], R.T), (-1,))

        pos_out = np.reshape(np.dot(d_out['positions'], R.T), (-1,))

        elems = (pos_intra_inp, pos_inter_inp, pos_out)
        ...
        return elems, energy_ndx_out, energy_ndx_inp
...
class GAN():
    ...
    def __init__(self, device, cfg):
        ...
        self.feature_dim = self.n_env_mols *3*3
        """
        ...
        if self.z_dim != 0:
            ...
            self.generator = model.G_dense(n_input=self.feature_dim,
                                            n_output=self.target_dim,
                                            start_channels=self.cfg.getint('model', 'n_chns'),
                                            fac=1,
                                            sn=self.cfg.getint('model', 'sn_gen'),
        ...
        if cfg.getint('grid', 'resolution') == 8:
            self.critic = model.C_dense(in_channels=self.critic_dim,
                                              start_channels=self.cfg.getint('model', 'n_chns'),
    ...
    def get_energies(self, coords, energy_ndx):
    ...
    def dstr(self, coords, energy_ndx):
    ...
    def dstr_loss(self, real_atom_grid, fake_atom_grid, energy_ndx_real, energy_ndx_fake):
        ...
        bond_ndx_real, angle_ndx_real, dih_ndx_real, lj_ndx_real = energy_ndx_real
        bond_ndx_fake, angle_ndx_fake, dih_ndx_fake, lj_ndx_fake = energy_ndx_fake
    ...
    def train(self):
        ...
        for epoch in epochs:
            ...
            for train_batch in tqdm_train_iterator:
                ...
                elems,  energy_ndx_out, energy_ndx_inp = train_batch

                if n == n_critic:
                    for key, value in c_loss_dict.items():
                        self.out.add_scalar(key, value, global_step=self.step)
                    g_loss_dict = self.train_step_gen(elems, energy_ndx_out, energy_ndx_inp)
                    for key, value in g_loss_dict.items():
                        self.out.add_scalar(key, value, global_step=self.step)
                    #print(c_loss_dict)
                    tqdm_train_iterator.set_description('D: {:.2f}, G: {:.2f}, E_out: {:.2f}, {:.2f}, {:.2f}, {:.2f}, E_inp: {:.2f}, {:.2f}, {:.2f}, {:.2f}'.format(c_loss_dict['Critic/wasserstein'],
                    ...
                    if self.loader_val:
                        ...
                        elems, energy_ndx_out, energy_ndx_inp = val_batch
                        g_loss_dict = self.train_step_gen(elems, energy_ndx_out, energy_ndx_inp, backprop=False)
                        for key, value in g_loss_dict.items():
    ...
    def val(self, dir=None):
        ...
        try:
            ...
            for n in range(0, self.cfg.getint('validate', 'n_gibbs')):
                g = iter(Mol_Generator(self.data, train=False, res='inp', rand_rot=False))
                for d in g:
                    with torch.no_grad():
                        #batch = all_elems[ndx:min(ndx + val_bs, len(all_elems))]

                        inp_positions = np.array([d['positions_inter']])
                        ...
                        inp_positions = torch.reshape(inp_positions, (val_bs, -1))
                        ...
                        if self.z_dim != 0:
                            z = torch.empty(
                                [inp_positions.shape[0], self.z_dim],
                                dtype=torch.float32,
                                device=self.device,
                            ).normal_()

                            fake_mol = self.generator(z, inp_positions)
                        else:
                            fake_mol = self.generator(inp_positions)

                        coords = torch.reshape(fake_mol, (val_bs, -1, 3))
    ...
    def train_step_critic(self, elems):
        ...
        pos_intra_inp, pos_inter_inp, pos_out = elems

        #generate fake atom
        if self.z_dim != 0:
            z = torch.empty(
                [pos_inter_inp.shape[0], self.z_dim],
                dtype=torch.float32,
                device=self.device,
            ).normal_()

            fake_atom = self.generator(z, pos_inter_inp)
        else:
            fake_atom = self.generator(pos_inter_inp)

        fake_data = torch.cat([fake_atom, pos_inter_inp], dim=1)
        real_data = pos_out
    ...
    def train_step_gen(self, elems, energy_ndx_out, energy_ndx_inp, backprop=True):
        ...
        #critic
        fake_positions = torch.cat([fake_atom, pos_inter_inp], dim=1)
        critic_fake = self.critic(fake_positions)
        ...
        fake_positions = torch.reshape(fake_positions, (self.bs, -1, 3))
        pos_out = torch.reshape(pos_out, (self.bs, -1, 3))

        e_bond_out, e_angle_out, e_dih_out, e_lj_out = self.get_energies(fake_positions, energy_ndx_out)
        e_bond_inp, e_angle_inp, e_dih_inp, e_lj_inp = self.get_energies(pos_out, energy_ndx_inp)

        b_dstr_real, a_dstr_real, d_dstr_real, nb_dstr_real = self.dstr(pos_out, energy_ndx_out)
        b_dstr_fake, a_dstr_fake, d_dstr_fake, nb_dstr_fake = self.dstr(fake_positions, energy_ndx_inp)

        if self.step % 50 == 0:
            fig = plt.figure()
            ax = plt.gca()
            x = [h * 0.4 / self.n_bins for h in range(0, self.n_bins)]
            ax.plot(x, b_dstr_real.detach().cpu().numpy()[:, 0], label='real')
            ...
            fig = plt.figure()
            ax = plt.gca()
            x = [h * 180.0 / self.n_bins for h in range(0, self.n_bins)]
            ax.plot(x, a_dstr_real.detach().cpu().numpy()[:, 0], label='real')
            ...
            ax = plt.gca()
            x = [h * 180.0 / self.n_bins for h in range(0, self.n_bins)]
            ax.plot(x, d_dstr_real.detach().cpu().numpy()[:, 0], label='real')
            ...
            ax = plt.gca()
            x = [h * 2.0/64 for h in range(0,64)]
            ax.plot(x, nb_dstr_real.detach().cpu().numpy()[:,0], label='real')
