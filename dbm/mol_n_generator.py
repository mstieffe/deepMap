...
class Mol_N_Generator():
    ...
    def __init__(self, data, train=False, rand_rot=False, n_mols=3):
        ...
        #self.mols_inp = [self.mols_inp[x:x+self.n_mols] for x in range(0, len(self.mols_inp), self.n_mols)]
        #self.mols_out = [self.mols_out[x:x+self.n_mols] for x in range(0, len(self.mols_out), self.n_mols)]
    ...
    def __iter__(self):
        ...
        for mol_inp, mol_out in zip(self.mols_inp, self.mols_out):
            ...
            batch_mol_out = [mol_out] + list(mol_out.env_mols[:self.n_mols-1])
            env_mol_inp = list(mol_inp.env_mols[self.n_mols-1:])
            env_mol_out = list(mol_out.env_mols[self.n_mols-1:])

            positions_intra_inp, positions_intra_out, atoms_inp, atoms_out = [], [], [], []
            ...
            for mol in env_mol_out:
                for a in mol.atoms:
                    env_atoms_out.append(a)
            ...
            atoms = list(atoms_inp) + list(env_atoms_inp)
            beads = list(atoms_out) + list(env_atoms_out)

            #energy ndx
            inp_intra_index_dict = dict(zip(atoms_inp, range(0, len(atoms_inp))))
            out_intra_index_dict = dict(zip(atoms_out, range(0, len(atoms_out))))
            inp_index_dict = dict(zip(atoms, range(0, len(atoms))))
            out_index_dict = dict(zip(beads, range(0, len(beads))))

            inp_bond_ndx, inp_ang_ndx, inp_dih_ndx, inp_lj_ndx = self.energy_ndx(batch_mol_inp, inp_intra_index_dict, inp_index_dict)
            out_bond_ndx, out_ang_ndx, out_dih_ndx, out_lj_ndx = self.energy_ndx(batch_mol_out, out_intra_index_dict, out_index_dict)

            #features (atom types)
            inp_intra_featvec = self.featvec(atoms_inp, self.data.ff_inp)
            inp_inter_featvec = self.featvec(env_atoms_inp, self.data.ff_inp)

            out_intra_featvec = self.featvec(atoms_out, self.data.ff_out)
    ...
    def energy_ndx(self, mols, index_intra_dict, index_dict):
        bond_ndx = []
        for mol in mols:
            for b in mol.bonds:
