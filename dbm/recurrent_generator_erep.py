            ...
            for bead in bead_seq:
                ...
                if self.pad_seq:
                    for n in range(0, self.data.max['seq_len'] - len(atom_seq_dict[bead])):
                        ...
                        bonds_ndx_pa.append(np.ones(np.array(bonds_ndx_pa[-1]).shape)*-1)
                        angles_ndx1_pa.append(np.ones(np.array(angles_ndx1_pa[-1]).shape)*-1)
                        angles_ndx2_pa.append(np.ones(np.array(angles_ndx2_pa[-1]).shape)*-1)
                        dihs_ndx_pa.append(np.ones(np.array(dihs_ndx_pa[-1]).shape)*-1)
                        ljs_ndx_pa.append(np.ones(np.array(ljs_ndx_pa[-1]).shape)*-1)
