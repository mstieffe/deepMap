...
from dbm.mol_rec_generator2 import Mol_Rec_Generator
...
class DS(Dataset):
    def __init__(self, data, cfg, train=True):
        ...
        g_out = Mol_Rec_Generator(data, train=train, res='out', rand_rot=False)
        g_inp = Mol_Rec_Generator(data, train=train, res='inp', rand_rot=False)
...
class GAN():
    ...
    def train(self):
        ...
        for epoch in epochs:
            ...
            for train_batch in tqdm_train_iterator:
                ...
                elems, initial, energy_ndx_out, energy_ndx_inp = train_batch
                elems = self.transpose_and_zip(elems)

                if n == n_critic:
                    for key, value in c_loss_dict.items():
                        self.out.add_scalar(key, value, global_step=self.step)
                    g_loss_dict = self.train_step_gen(elems, initial, energy_ndx_out, energy_ndx_inp)
                    ...
                    if self.loader_val:
                        ...
                        elems, initial, energy_ndx_out, energy_ndx_inp = val_batch
                        elems = self.transpose_and_zip(elems)
                        g_loss_dict = self.train_step_gen(elems, initial, energy_ndx_out, energy_ndx_inp, backprop=False)
                        for key, value in g_loss_dict.items():
    ...
    def train_step_critic(self, elems, initial):
        ...
        real_atom_grid, fake_atom_grid = initial
        ...
        for target_atom, featvec_real, repl_real, featvec_fake, repl_fake in elems:
            ...
            #update aa grids
            fake_atom_grid = torch.where(repl_fake[:,:,None,None,None], fake_atom_grid, fake_atom)
            real_atom_grid = torch.where(repl_real[:,:,None,None,None], real_atom_grid, target_atom[:, None, :, :, :])
    ...
    def train_step_gen(self, elems, initial, energy_ndx_out, energy_ndx_inp, backprop=True):
        ...
        for target_atom, _, _, featvec, repl in elems:
            #prepare input for generator
        ...
        e_bond_out, e_angle_out, e_dih_out, e_lj_out = self.get_energies(fake_atom_grid, energy_ndx_out)
        e_bond_inp, e_angle_inp, e_dih_inp, e_lj_inp = self.get_energies(real_atom_grid, energy_ndx_inp)
